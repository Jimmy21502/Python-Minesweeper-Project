...
class Minesweeper:
    ...
    def __init__(self):
        ...
        self.easy_seconds_passed = None
        self.medium_seconds_passed = None
        self.hard_seconds_passed = None
        self.custom_seconds_passed = None
        self.ghostsweeper_seconds_left = None
...
click on every tile that does not contain a bomb. (Click to continue)""")
    ...
    def create_easy_frame(self):
        ...
        for i in range(EASY_GRID):
            ...
            for j in range(EASY_GRID):
                ...
                easy_tile.bind('<Button-1>', lambda event, x=i, y=j: self.left_click(self.easy_clicks, x, y, EASY_GRID, EASY_GRID, self.easy_buttons, EASY_TOTAL_TILES, EASY_TOTAL_BOMBS, self.easy_timer_count, self.easy_seconds_passed))
    ...
    def create_medium_frame(self):
        ...
        for i in range(MEDIUM_GRID):
            ...
            for j in range(MEDIUM_GRID):
                ...
                medium_tile.bind('<Button-1>', lambda event, x=i, y=j: self.left_click(self.medium_clicks, x, y, MEDIUM_GRID, MEDIUM_GRID, self.medium_buttons, MEDIUM_TOTAL_TILES, MEDIUM_TOTAL_BOMBS, self.medium_timer_count, self.medium_seconds_passed))
    ...
    def create_hard_frame(self):
        ...
        for i in range(HARD_GRID):
            ...
            for j in range(HARD_GRID):
                ...
                hard_tile.bind('<Button-1>', lambda event, x=i, y=j: self.left_click(self.hard_clicks, x, y, HARD_GRID, HARD_GRID, self.hard_buttons, HARD_TOTAL_TILES, HARD_TOTAL_BOMBS, self.hard_timer_count, self.hard_seconds_passed))
    ...
    def create_custom_frame(self):
        ...
        for i in range(self.custom_rows):
            ...
            for j in range(self.custom_columns):
                ...
                custom_tile.bind('<Button-1>', lambda event, x=i, y=j: self.left_click(self.custom_clicks, x, y, self.custom_rows, self.custom_columns, self.custom_buttons, (self.custom_columns*self.custom_rows), self.custom_total_bombs, self.custom_timer_count, self.custom_seconds_passed))
    ...
    def fruit_clicks(self, i, j):
        ...
        if self.fruit_bombs == 0:
            ...
            self.ask_player_name("fruitsweeper", current_points)
    ...
    def create_ghostsweeper_frame(self):
        ...
        for i in range(GHOSTSWEEPER_GRID):
            ...
            for j in range(GHOSTSWEEPER_GRID):
                ...
                ghostsweeper_tile.bind('<Button-1>', lambda event, x=i, y=j: self.left_click(self.ghostsweeper_clicks, x, y, GHOSTSWEEPER_GRID, GHOSTSWEEPER_GRID, self.ghostsweeper_buttons, GHOSTSWEEPER_TOTAL_TILES, GHOSTSWEEPER_TOTAL_BOMBS, self.ghostsweeper_timer_count, self.ghostsweeper_seconds_left))
...
Use your lights sparingly, and reveal every tile to win!""")
    ...
    def create_leaderboard_frame(self):
        ...
        self.lb_menu = tk.Frame(self.leaderboard_frame)
        self.lb_menu.grid(row=0, column=0)

        easy_lb_button = tk.Button(self.lb_menu, text="Easy Leaderboard", command = lambda: self.show_leaderboard("easy"))
        easy_lb_button.grid(row=0, column=0)

        medium_lb_button = tk.Button(self.lb_menu, text="Medium Leaderboard", command = lambda: self.show_leaderboard("medium"))
        medium_lb_button.grid(row=1, column=0)

        hard_lb_button = tk.Button(self.lb_menu, text="Hard Leaderboard", command = lambda: self.show_leaderboard("hard"))
        hard_lb_button.grid(row=2, column=0)

        fruitsweeper_lb_button = tk.Button(self.lb_menu, text="Fruitsweeper Leaderboard", command = lambda: self.show_leaderboard("fruitsweeper"))
        fruitsweeper_lb_button.grid(row=3, column=0)

        back_button = tk.Button(self.lb_menu, text="Back", command = lambda: self.show_frame("MainFrame"))
        back_button.grid(row=4, column=0)

        self.lb_scores = tk.Frame(self.leaderboard_frame)
        self.lb_scores.grid(row=1, column=0)

        self.leaderboard_frame.grid_rowconfigure(5, weight = 1)
        self.leaderboard_frame.grid_columnconfigure(0, weight = 1)      

        return self.leaderboard_frame
...
(Click on the tile highlighted green to continue)""")
    ...
    def show_leaderboard(self, mode):
        ...
        self.lb_scores = tk.Frame(self.leaderboard_frame)
        self.lb_scores.grid(row=0, column=0)
        ...
        for name, time in scores[:10]:
            if mode == "fruitsweeper":
                score_label = tk.Label(self.lb_scores, text=f"{row}. {name} - {time} Points")
                score_label.grid(row=row, column=0)
            else:
                score_label = tk.Label(self.lb_scores, text=f"{row}. {name} - {time}s")
                score_label.grid(row=row, column=0)
            row += 1
    ...
    def ask_player_name(self, mode, time):
        ...
        name_label = tk.Label(popup, text="Enter your name for Leaderboard:")
        ...
        def submit():
            ...
            if name:
                self.save_score(mode, name, time)
            popup.destroy()

        submit_button = tk.Button(popup, text="Submit", command = submit)
        submit_button.grid(row=2, column=0)

        skip_button = tk.Button(popup, text="Skip", command = popup.destroy)
